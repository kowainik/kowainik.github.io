...
The blog post uses the following approach supported by `co-log`.
...
This data type looks scary but the idea is simple: it just stores `LogAction msg
m` in the `ReaderT` environment.
...
`cbind` function and see how it helps:
...
```haskell
contramap f (LogAction action) = LogAction (action  .  f)
cbind     f (LogAction action) = LogAction (action <=< f)
```
...
* [Kowainik: typerep-map step by step](https://kowainik.github.io/posts/2018-07-11-typerep-map-step-by-step.html)
